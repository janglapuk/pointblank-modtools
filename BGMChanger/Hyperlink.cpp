...
void CMyHyperLink::SetTootTipText(LPCSTR szToolTip)
{
	if (m_bEnableToolTip )
	{
		m_ToolTip.UpdateTipText(CString(szToolTip),this,1001);
	}
...
void CMyHyperLink::PreSubclassWindow() 
{
	...
	GetWindowTextA(szCurretText, MAX_PATH);
	...
	GetWindowsDirectoryA(szWindowsDir ,MAX_PATH*2);
	strcat(szWindowsDir,"\\Winhlp32.exe");
	HMODULE hModule = LoadLibraryA(szWindowsDir);
